...
| **1** | One floor plan, 5 versions (V1→V5), degrading — a mappable 4-room house each time: kitchen hub + front hall + scullery + pantry (Change #12) | V1→V5 in order | The paired windows (V3), the pre-echoed mark (V4) — Change #16 | All 4 exterior doors (front, back, press-house, pantry) always advance to the next version's kitchen — no dead ends, no doubling back. **Pantry is exempt from folding entirely** (always → next, the reliable line). **Front/back/press-house are each one growing fold-set per kind**: 1st use of a kind advances normally, 2nd use of that same kind sends them back to whichever other instance is already open, 3rd+ is random among all open (Change #14/#15) → V5's kitchen (**Four Casses** — all 4 doors collapsed into one room, first time that's happened — Change #13) → D2 |
