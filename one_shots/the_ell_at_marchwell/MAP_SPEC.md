...
| **1** | 5 copies of a mappable 4-room house each (kitchen hub + front hall + scullery + pantry — Change #12), degrading | Copies 1→5 in order | The paired windows, the pre-echoed mark (Change #16) — either copy, DM's pick | Front hall's front door · scullery's back door and press-house door · the pantry door — all 4 → next copy, forward use never fold-governed; going *back* through a used exterior door does fold (pantry exempt — Change #15) → Copy 5's kitchen (**Four Casses** — all 4 exterior doors collapsed into one room, first time that's happened — Change #13) → D2 |
