...
- **Cartography Determinism Cleanup + Strict pass (2026-06-18):** the deterministic counts a mapper should expect are **18 standalone map-authoritative regions** (Cindern Waste is a terrain sub-zone of Emberfell, not a region; the Drowned Steps is a sub-area of Sallowmarch), **4 major cities** (Caradril AUTHORITATIVE; Glassmere/Calderport/Ashfast layouts now **DERIVED_CANON** and authoritative for rendering), **40 settlement packets**, and **36 adventure sites** with consistent D-IDs. Tollreach is a **danger-icon route waypoint** (75,37), not a labeled service settlement. All D-site IDs across region/settlement packets and the master index are now consistent. Strict pass final counts: 57 AUTHORITATIVE, 72 DERIVED_CANON, 7 NOT_MAP_AUTHORITATIVE, 0 CARTOGRAPHY_BLOCKER.
