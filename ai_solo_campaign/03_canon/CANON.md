...
Authoritative record of all established world facts. When any content contradicts this file, this file wins unless a formal revision is documented here. Mixed secrecy — some canon is player-safe, some DM-only. See `PLAYER_SAFE_CANON.md` and `DM_ONLY_CANON.md` for separated views.
...
- Check this file before adding new world facts to avoid contradictions.
- Add every newly established major fact here.
- Mark each fact as player-safe or DM-only.
- Never delete established canon — if a fact must change, document the revision with a reason.
