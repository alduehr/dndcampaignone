...
- [ ] Begin Stage 11: Mystery, Secret, and Clue Expansion
  - Why it matters: **Stage 10 (Quest Library Expansion) is complete** — Stage 11 is next per `DEVELOPMENT_STAGES.md`. The quest library now spans the whole continent (28 major campaign quests, ~165 developed quests, 304 far-continent hooks/rumors/jobs on top of ~281 home-region hooks/rumors). Stage 10 seeded many **oblique far-region mystery-fragments** (M-line echoes, the Mourner old-songs network nodes, the Saltmere "going-down," the steppe "thinning sky," the Sunmark "far root-ache" hopeful contrast, the Crown's "vanished inner circle," the Emberfell mirror). Stage 11 should lift these into `CLUE_INDEX.md` / `MYSTERY_WEB.md` / `REVELATION_MAP.md`, confirm every major conclusion has ≥3 clue paths, and verify the apex truth stays gated. Expand `SECRET_INDEX`, `MYSTERY_CHAINS`, `FALSE_LEADS`, `PROPHECIES_AND_OMENS`, `REVEAL_TIMING`.
  - Related files: `11_mysteries_and_secrets/`, `09_quests/by_region/`, `09_quests/hooks_and_rumors/fc_*`, `02_runtime_state/KNOWN_CLUES.md` / `HIDDEN_CLUES.md`
  - Suggested agent: `mystery-clue-engineer`
  - Stage: 11
  - Blocker: none — Stage 10 complete.
