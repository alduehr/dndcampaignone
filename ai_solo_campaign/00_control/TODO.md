...
- [ ] Generate the actual map images (geometry and settlements now fully exploration-deterministic; all packets complete and counts reconciled)
  - Why it matters: cartographic canon, prompts, AND deterministic geometry are now ready; only the rendered images are missing. **The Cartography Authority Pass (2026-06-16) + Determinism Cleanup (2026-06-18) resolved all geography into explicit coordinates and reconciled counts** — `CARTOGRAPHY_AUTHORITY_FULL_CONTINENT.md` (master geometry), `MAP_FEATURE_REGISTRY.md` (every feature + four-status taxonomy; 3 far cities DERIVED_CANON), the route/water/terrain authority files, **18 region map packets, 4 city packets, 40 settlement packets**, and `ADVENTURE_SITE_CARTOGRAPHY_INDEX.md` (D01–D36, all IDs consistent). A mapper can now render the continent, every region, the 4 cities, every important settlement, and all sites without guessing. **Recommended order:** `FULL_WORLD_MAP_PROMPTS.md` Prompt 5 (campaign-area standalone zoom = the day-to-day play map) first; then Prompt 1 (player-safe full continent); then Prompt 3 (parchment artifact for the player); then per-region/city packets; then Prompt 2 / Prompt 4-toggle (DM-only). For an external image model that cannot read repo files, use `PLAYER_SAFE_FULL_CONTINENT_GENERATION_PACKET.md`. **Never render the DM-only apex (Concord Deep / Under-Shrine / keystone / Hollow Court / D23) on a player map.**
