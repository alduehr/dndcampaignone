...
- [ ] Generate the actual map images (geometry and settlements now fully exploration-deterministic; all 42 packets complete)
  - Why it matters: cartographic canon, prompts, AND deterministic geometry are now ready; only the rendered images are missing. **The Cartography Authority Pass (2026-06-16) resolved all geography into explicit coordinates** — `CARTOGRAPHY_AUTHORITY_FULL_CONTINENT.md` (master geometry), `MAP_FEATURE_REGISTRY.md` (every feature + confidence), the route/water/terrain authority files, 19 region map packets, 4 city packets, 18 settlement packets, and `ADVENTURE_SITE_CARTOGRAPHY_INDEX.md` (D01–D36). A mapper can now render the continent, every region, the 4 cities, every important settlement, and all sites without guessing. **Recommended order:** `FULL_WORLD_MAP_PROMPTS.md` Prompt 5 (campaign-area standalone zoom = the day-to-day play map) first; then Prompt 1 (player-safe full continent); then Prompt 3 (parchment artifact for the player); then per-region/city packets; then Prompt 2 / Prompt 4-toggle (DM-only). For an external image model that cannot read repo files, use `PLAYER_SAFE_FULL_CONTINENT_GENERATION_PACKET.md`. **Never render the DM-only apex (Concord Deep / Under-Shrine / keystone / Hollow Court / D23) on a player map.**
  - Related files: `04_world_atlas/CARTOGRAPHY_AUTHORITY_FULL_CONTINENT.md`, `MAP_FEATURE_REGISTRY.md`, `FULL_WORLD_MAP_PROMPTS.md`, `PLAYER_SAFE_FULL_CONTINENT_GENERATION_PACKET.md`; region/city/settlement packets; `18_audits/CARTOGRAPHY_READINESS_AUDIT.md`
  - Suggested agent: world-atlas-builder (or external image tool)
  - Stage: interstitial (post-Cartography-Authority-Pass)
