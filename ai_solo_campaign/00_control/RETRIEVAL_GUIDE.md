...
- `04_world_atlas/FULL_WORLD_MAP_LAYERS.md` — 8 full-world map layers (incl. a campaign-area standalone zoom map). **Layers 2, 4, & 7 are DM-only — never render for the player.**
- `04_world_atlas/FULL_WORLD_MAP_PROMPTS.md` — 5 full-world image prompts. **Prompts 1, 3, & 5 are player-safe; Prompt 2 and Prompt 4's DM toggles are DM-only.** The campaign-area map (Prompt 5) is a separate standalone zoom map, not an inset embedded into the player-safe full-continent map (Prompt 1). Recommended first map overall: Prompt 5 (campaign-area standalone zoom map); then Prompt 1 (player-safe continent); then Prompt 3 (parchment artifact). **Prompt 1 is repo-aware** (uses the render manifest); for an external image model, use the generation packet below.
- `04_world_atlas/PLAYER_SAFE_FULL_CONTINENT_RENDER_MANIFEST.md` — **label authority for the player-safe full-continent map.** Load before generating or auditing any player-facing full-continent map. Contains canonical feature table, duplicate rules, route/river geometry, region footprints, post-generation checklist.
- `04_world_atlas/PLAYER_SAFE_FULL_CONTINENT_GENERATION_PACKET.md` — **self-contained copy-paste prompt** for external image generators that cannot read the repo (Sections A–K). Use this instead of Prompt 1 when the image model has no repo access.
